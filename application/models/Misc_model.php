...
class Misc_model extends App_Model
{
    ...
    public function perform_search($q)
    {
        ...
        $purchases_order_search = $this->_search_purchase_orders($q, $limit);
        if (count($purchases_order_search['result']) > 0) {
            $result[] = $purchases_order_search;
        }
        $purchases_vendors_search = $this->_search_purchase_vendors($q, $limit);
        if (count($purchases_vendors_search['result']) > 0) {
            $result[] = $purchases_vendors_search;
        }
    ...
    public function _search_purchase_vendors($q, $limit = 0)
    {
            $result = [
                'result'         => [],
                'type'           => 'pur_vendor',
                'search_heading' => _l('vendors'),
            ];
    ...
    public function _search_items($q, $limit = 0)
    {
            ...
            $this->db->where('(description LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\' 
                OR commodity_code LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\' 
                OR ' . db_prefix() . 'items_groups.name LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                OR commodity_barcode LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\')');
