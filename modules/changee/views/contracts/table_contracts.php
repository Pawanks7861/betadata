...
if(!has_permission('changee_contracts', '', 'view')){
...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == db_prefix().'pur_contracts.vendor'){
            $ven = changee_get_vendor_company_name($aRow[db_prefix().'pur_contracts.vendor']);
            $_data = '<a href="' . admin_url('changee/vendor/' . $aRow[db_prefix().'pur_contracts.vendor']) . '" >' .  $ven . '</a>';
        }elseif($aColumns[$i] == 'contract_name'){
            $numberOutput = '';
            $numberOutput = '<a href="' . admin_url('changee/contract/' . $aRow['contract_id']) . '">' . $aRow['contract_number'].' - '. $aRow['contract_name'] . '</a>';
    
            $numberOutput .= '<div class="row-options">';

            if (has_permission('changee_contracts', '', 'view') || has_permission('changee_contracts', '', 'view_own')) {
                $numberOutput .= ' <a href="' . admin_url('changee/contract/' . $aRow['contract_id']) . '" >' . _l('view') . '</a>';
            }
            if (has_permission('changee_contracts', '', 'edit')) {
                $numberOutput .= ' | <a href="' . admin_url('changee/contract/' . $aRow['contract_id']) . '">' . _l('edit') . '</a>';
            }
            if (has_permission('changee_contracts', '', 'delete')) {
                $numberOutput .= ' | <a href="' . admin_url('changee/delete_contract/' . $aRow['contract_id']) . '" class="text-danger">' . _l('delete') . '</a>';
            }
            $numberOutput .= '</div>';
