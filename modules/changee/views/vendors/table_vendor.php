...
$hasPermissionDelete = has_permission('changee_vendors', '', 'delete');
...
$groups   = $this->ci->changee_model->get_vendor_category();
$groupIds = [];
...
foreach ($rResult as $key => $aRow) {
    ...
    $url = admin_url('changee/vendor/' . $aRow['userid']);
    ...
    if ($aRow['registration_confirmed'] == 0 && is_admin()) {
        $company .= ' | <a href="' . admin_url('changee/confirm_registration/' . $aRow['userid']) . '" class="text-success bold">' . _l('confirm_registration') . '</a>';
    }
    if (!$isPerson) {
        $company .= ' | <a href="' . admin_url('changee/vendor/' . $aRow['userid'] . '?group=contacts') . '">' . _l('customer_contacts') . '</a>';
    }
    if ($hasPermissionDelete) {
        $company .= ' | <a href="' . admin_url('changee/delete_vendor/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    ...
    if ($aRow[db_prefix().'pur_vendor.category']) {
        ...
        foreach ($groups as $group) {
            $groupsRow .= '<span class="label label-default mleft5 inline-block">' . changee_get_vendor_cate_name_by_id($group) . '</span>';
        }
    }
    ...
    <input type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'changee/change_vendor_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow[db_prefix().'pur_vendor.active'] == 1 ? 'checked' : '') . '>
