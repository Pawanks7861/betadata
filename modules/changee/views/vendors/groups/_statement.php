         ...
         <tr>
           ...
           <td>
            ...
            if(isset($data['invoice_id'])) {
              echo _l('statement_invoice_details',array('<a href="'.admin_url('changee/changee_invoice/'.$data['invoice_id']).'" target="_blank">'.changee_get_pur_invoice_number($data['invoice_id']).'</a>',_d($data['duedate'])));
            } else if(isset($data['payment_id'])){
             echo _l('statement_payment_details',array('<a href="'.admin_url('changee/payment_invoice/'.$data['payment_id']).'" target="_blank">'.'#'.$data['payment_id'].'</a>',changee_get_pur_invoice_number($data['payment_invoice_id'])));
           } else if(isset($data['debit_note_id'])) {
            echo _l('statement_debit_note_details',array('<a href="'.admin_url('changee/debit_notes/'.$data['debit_note_id']).'" target="_blank">'.changee_format_debit_note_number($data['debit_note_id']).'</a>'));
          } else if(isset($data['debit_id'])) {
            echo _l('statement_debits_applied_details',array(
              '<a href="'.admin_url('changee/debit_notes/'.$data['debit_applied_debit_note_id']).'" target="_blank">'.changee_format_debit_note_number($data['debit_applied_debit_note_id']).'</a>',
            ...
            echo _l('statement_debit_note_refund', changee_format_debit_note_number($data['refund_debit_note_id']));
          }
          ?>
        </td>
