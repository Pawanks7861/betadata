 ...
 <thead>
  ...
  <tbody>
    ...
    <tr>
        ...
        <td>
            <?php if (has_permission('changee_settings', '', 'edit') || is_admin()) { ?>
              <a href="#" onclick="edit_commodity_group_type(this,<?php echo changee_pur_html_entity_decode($commodity_group_type['id']); ?>); return false;" data-commodity_group_code="<?php echo changee_pur_html_entity_decode($commodity_group_type['commodity_group_code']); ?>" data-name="<?php echo changee_pur_html_entity_decode($commodity_group_type['name']); ?>" data-order="<?php echo changee_pur_html_entity_decode($commodity_group_type['order']); ?>" data-display="<?php echo changee_pur_html_entity_decode($commodity_group_type['display']); ?>" data-note="<?php echo changee_pur_html_entity_decode($commodity_group_type['note']); ?>" class="btn btn-default btn-icon"><i class="fa fa-pencil-square"></i>
            </a>
            <?php } ?>

            <?php if (has_permission('changee_settings', '', 'edit') || is_admin()) { ?> 
            <a href="<?php echo admin_url('changee/delete_commodity_group_type/'.$commodity_group_type['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
             <?php } ?>
        </td>
    </tr>
    <?php } ?>
...
<div class="modal1 fade" id="commodity_group_type" tabindex="-1" role="dialog">
        <div class="modal-dialog setting-handsome-table">
          <?php echo form_open_multipart(admin_url('changee/commodity_group_type'), array('id'=>'add_commodity_group_type')); ?>
