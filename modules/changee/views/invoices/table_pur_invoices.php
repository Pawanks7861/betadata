...
if(!has_permission('changee_invoices', '', 'view')){
...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'invoice_number'){
            ...
            $numberOutput = '<a href="' . admin_url('changee/changee_invoice/' . $aRow['id']) . '"  >'.$aRow['invoice_number']. '</a>';
            
            $numberOutput .= '<div class="row-options">';

            if (has_permission('changee_invoices', '', 'view') || has_permission('changee_invoices', '', 'view_own')) {
                $numberOutput .= ' <a href="' . admin_url('changee/changee_invoice/' . $aRow['id']) . '" >' . _l('view') . '</a>';
            }
            if ((has_permission('changee_invoices', '', 'edit') || is_admin()) ) {
                $numberOutput .= ' | <a href="' . admin_url('changee/pur_invoice/' . $aRow['id']) . '">' . _l('edit') . '</a>';
            }
            if (has_permission('changee_invoices', '', 'delete') || is_admin()) {
                $numberOutput .= ' | <a href="' . admin_url('changee/delete_pur_invoice/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            $numberOutput .= '</div>';
            ...
            $tax = $this->ci->changee_model->get_html_tax_pur_invoice($aRow['id']);
            $total_tax = 0;
            ...
            $_data = '<a href="'.admin_url('changee/contract/'.$aRow['contract']).'">'.$aRow['contract_number'].'</a>';
        }elseif($aColumns[$i] == 'payment_request_status'){
            $_data = changee_get_payment_request_status_by_inv($aRow['id']);
        }elseif($aColumns[$i] == db_prefix().'pur_invoices.pur_order'){
            $_data = '<a href="'.admin_url('changee/changee_order/'.$aRow[db_prefix().'pur_invoices.pur_order']).'">'.changee_get_pur_order_subject($aRow[ db_prefix().'pur_invoices.pur_order']).'</a>';
        }elseif($aColumns[$i] == 'is_recurring_from'){
            $_data = ' <a href="' . admin_url('changee/changee_invoice/' . $aRow['is_recurring_from']) . '" >' . changee_get_pur_invoice_number($aRow['is_recurring_from']) . '</a>'; 
        }elseif($aColumns[$i] == db_prefix().'pur_invoices.vendor'){
            $_data = '<a href="' . admin_url('changee/vendor/' . $aRow[db_prefix().'pur_invoices.vendor']) . '" >' .  changee_get_vendor_company_name($aRow[db_prefix().'pur_invoices.vendor']) . '</a>'; 
        }else {
