...
<div id="wrapper">
  <div class="content">
    ...
    <?php echo form_open_multipart(admin_url('changee/pur_invoice_form'),array('id'=>'pur_invoice-form','class'=>'_pur_invoice_form _transaction_form')); ?>
    	<?php
    		if(isset($pur_order)){
      ...
      <div class="col-md-12">
        <div class="panel_s accounting-template estimate">
          <div class="panel-body">
          	...
          	<div class="row">
             <div class="col-md-12">
              <h4 class="no-margin font-bold"><i class="fa <?php if(isset($pur_invoice)){ echo 'fa-pencil-square';}else{ echo 'fa-plus';} ?>" aria-hidden="true"></i> <?php echo _l($title); ?> <?php if(isset($pur_invoice)){ echo ' '.changee_pur_html_entity_decode($pur_invoice->invoice_number); } ?></h4>
              <hr />
             </div>
            </div>
	                ...
	                <div class="col-md-6 pad_left_0 form-group">
	                    ...
	                    <select name="vendor" id="vendor" class="selectpicker" onchange="pur_vendor_change(this); return false;" required="true" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
	                        ...
	                        <?php foreach($vendors as $ven){ ?>
	                        	<option value="<?php echo changee_pur_html_entity_decode($ven['userid']); ?>" <?php if(isset($pur_invoice) && $pur_invoice->vendor == $ven['userid']){ echo 'selected'; } ?>><?php echo changee_pur_html_entity_decode($ven['vendor_code'].' '.$ven['company']); ?></option>
	                        <?php } ?>
	                    </select>
	                </div>
	                ...
	                <div class="col-md-6 form-group pad_right_0">
	                    ...
	                    <select name="contract" id="contract" class="selectpicker" onchange="contract_change(this); return false;" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
	                        ...
	                        <?php foreach($contracts as $ct){ ?>
	                        	<option value="<?php echo changee_pur_html_entity_decode($ct['id']); ?>" <?php if(isset($pur_invoice) && $pur_invoice->contract == $ct['id']){ echo 'selected'; } ?>><?php echo changee_pur_html_entity_decode($ct['contract_number']); ?></option>
	                        <?php } ?>
	                    </select>
	                </div>
					...
					<div class="col-md-6 pad_right_0">
	                	...
	                	<select name="project_id" id="project" class="selectpicker" data-live-search="true" data-width="100%" required="true" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
                          ...
                          <?php foreach ($projects as $s) { ?>
                            <option value="<?php echo changee_pur_html_entity_decode($s['id']); ?>" <?php if (isset($pur_invoice) && $s['id'] == $pur_invoice->project_id) {                                                                                              echo 'selected';
		        ...
		        <div class="row">
		          <div class="col-md-4">
		            <?php $this->load->view('changee/item_include/main_item_select'); ?>
		                ...
		                if(isset($pur_invoice) && $pur_invoice->currency != 0){
		                  $po_currency = changee_pur_get_currency_by_id($pur_invoice->currency);
		                } 
...
<?php require 'modules/changee/assets/js/pur_invoice_js.php';?>
