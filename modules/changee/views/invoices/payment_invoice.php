             ...
             <?php $csrf = array(
              ...
              <input type="hidden" id="csrf_token_name" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" />
          	   <?php if($payment_invoice->approval_status == 1){ ?>
                    <div class="ribbon info"><span class="fontz9" ><?php echo _l('changee_not_yet_approve'); ?></span></div>
          	...
          	<h4 class="pull-left "><?php echo _l('payment_for').' '; ?><a href="<?php echo admin_url('changee/changee_invoice/'. $payment_invoice->pur_invoice); ?>"><?php echo changee_pur_html_entity_decode($invoice->invoice_number); ?></a></h4>
				...
				<div class="row">
							...
							<div class="col-md-6">
								...
								<span class="pull-right bold">
									...
									<?php if(!empty($payment_invoice->paymentmode)){
										echo  changee_get_payment_mode_name_by_id($payment_invoice->paymentmode);
									}
									?>
								</span></p>
								<?php if(!empty($payment_invoice->transactionid)) { ?>
									<hr />
									<p><?php echo _l('payment_transaction_id'); ?>: <span class="pull-right bold"><?php echo changee_pur_html_entity_decode($payment_invoice->transactionid); ?></span></p>
								<?php } ?>
							</div>
							<div class="clearfix"></div>
							<div class="col-md-6">
					...
					<table class="table table-borderd table-hover">
						<thead>
							...
							<tbody>
								<tr>
									<td><?php echo changee_get_pur_invoice_number($payment_invoice->pur_invoice); ?></td>
									...
									<?php if($invoice->payment_status != 'paid') { ?>
											<td class="text-danger">
												<?php echo app_format_money(changee_purinvoice_left_to_pay($invoice->id), $base_currency->name); ?>
											</td>
										<?php } ?>
									</tr>
								</tbody>
    ...
    <div class="col-md-6">
     <div class="panel_s">
      ...
      <div class="project-overview-right">
        ...
        <a data-toggle="tooltip" data-loading-text="<?php echo _l('wait_text'); ?>" class="btn btn-success lead-top-btn lead-view" data-placement="top" href="#" onclick="send_request_approve(<?php echo changee_pur_html_entity_decode($payment_invoice->id); ?>); return false;"><?php echo _l('send_request_approve_pur'); ?></a>
      <?php } }
                    ...
                    <li>
                      ...
                      <li>
                        <div class="row text-right col-md-12">
                          <a href="#" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="approve_request(<?php echo changee_pur_html_entity_decode($payment_invoice->id); ?>); return false;" class="btn btn-success mright15"><?php echo _l('approve'); ?></a>
                         <a href="#" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="deny_request(<?php echo changee_pur_html_entity_decode($payment_invoice->id); ?>); return false;" class="btn btn-warning"><?php echo _l('deny'); ?></a></div>
                      </li>
                   </ul>
                </div>
          <?php }
        ...
        <div class="modal-body">
         <p class="bold" id="signatureLabel"><?php echo _l('signature'); ?></p>
          ...
          <div class="modal-footer">
           ...
           <button onclick="sign_request(<?php echo changee_pur_html_entity_decode($payment_invoice->id); ?>);" data-loading-text="<?php echo _l('wait_text'); ?>" autocomplete="off" class="btn btn-success"><?php echo _l('e_signature_sign'); ?></button>
          </div>
...
<?php require 'modules/changee/assets/js/payment_invoice_js.php';?>