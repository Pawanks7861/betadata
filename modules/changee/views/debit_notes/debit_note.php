      ...
      <div class="row">
       ...
       <label for="vendorid"><?php echo _l('vendor'); ?></label>
        <select name="vendorid" id="vendorid" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" >
            ...
            <option value="<?php echo changee_pur_html_entity_decode($s['userid']); ?>" <?php if(isset($debit_note) && $debit_note->vendorid == $s['userid']){ echo 'selected'; }else{ if(isset($ven) && $ven == $s['userid']){ echo 'selected';} } ?>><?php echo changee_pur_html_entity_decode($s['company']); ?></option>
              <?php } ?>
        </select> 
      </div>
 <div class="row">
   <div class="col-md-12">
    ...
    <?php 
     include_once(module_views_path('changee', 'debit_notes/billing_and_shipping_template.php')); ?>
...
<?php if (isset($debit_note) || isset($add_items)) {
  ...
  foreach ($add_items as $item) {
    ...
    $debit_note_item_taxes = changee_get_debit_note_item_taxes($item['id']);
                              // passed like string
    if ($item['id'] == 0) {
...
<?php init_tail(); ?>
 <?php require 'modules/changee/assets/js/debit_note_js.php';?>  
 </body>
 </html>
