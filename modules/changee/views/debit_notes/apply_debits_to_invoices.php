...
<div class="modal fade apply-debits-to-invoice" id="apply_debits" data-debits-remaining="<?php echo $debit_note->remaining_debits; ?>" tabindex="-1" role="dialog" aria-labelledby="modalLabelApplyDebits">
  <div class="modal-dialog modal-lg" role="document">
    <?php echo form_open(admin_url('changee/apply_debits_to_invoices/'.$debit_note->id),array('id'=>'apply_debits_form')); ?>
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="modalLabelApplyDebits">
            <?php echo _l('apply_debits_form').' '.changee_format_debit_note_number($debit_note->id); ?>
        </h4>
    </div>
        ...
        <div class="table-responsive debits-table">
            ...
            <tbody>
                <?php foreach($available_debitable_invoices as $invoice) {
                    $_currency = changee_get_base_currency_pur();
                    if($invoice['invoice_currency'] != 0){
                        $_currency = changee_pur_get_currency_by_id($invoice['invoice_currency']);
                    }
                    ?>

                    <?php if($_currency->id == $debit_note->currency){ ?>
                        <tr>
                            <td><a href="<?php echo admin_url('changee/changee_invoice/'.$invoice['id']); ?>" target="_blank"><?php echo changee_pur_html_entity_decode($invoice['invoice_number']); ?></a></td>
...
<?php echo form_close(); ?>
 ...
 <?php require 'modules/changee/assets/js/apply_debits_to_invoice_js.php';?>  
