...
<?php if((changee_debits_can_be_applied_to_invoice($pur_invoice->payment_status) && $debits_available > 0)) { ?>
<!-- Modal Apply Credits -->
<div class="modal fade apply-debits-from-invoice" id="apply_debits" data-balance-due="<?php echo changee_purinvoice_left_to_pay($pur_invoice->id); ?>" tabindex="-1" role="dialog" aria-labelledby="modalLabelApplyDebits">
  <div class="modal-dialog modal-lg" role="document">
    <?php echo form_open(admin_url('changee/apply_debits/'.$pur_invoice->id),array('id'=>'apply_debits_form')); ?>
    <?php 
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="modalLabelApplyDebits">
            <?php echo changee_pur_html_entity_decode($pur_invoice->invoice_number); ?> - <?php echo _l('apply_debits'); ?>
        </h4>
    </div>
        ...
        <div class="table-responsive debits-table">
            ...
            <tbody>
                <?php foreach($open_debits as $debit) { ?>
                    <?php $debit_currency = changee_pur_get_currency_by_id($debit['currency']); ?>
                    <?php if($debit_currency->id == $base_currency->id){ ?>
                    <tr>
                        <td><a href="<?php echo admin_url('changee/debit_notes/'.$debit['id']); ?>" target="_blank"><?php echo changee_format_debit_note_number($debit['id']); ?></a></td>
                ...
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td class="invoice-balance-due">
                            <?php echo app_format_money(changee_purinvoice_left_to_pay( $pur_invoice->id), $base_currency->symbol); ?>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<?php echo form_close(); ?>
 ...
 <?php require 'modules/changee/assets/js/apply_invoice_debits_js.php';?>  
