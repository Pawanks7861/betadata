  ...
  <div class="content">
    <div class="row">
      <?php if ($view == '') { ?>
        ...
        if (isset($estimate)) {
          echo form_open(site_url('changee/vendors_portal/quotation_form/' . $estimate->id), array('autocomplete' => 'off', 'class' => '_transaction_form'));
        } else {
          echo form_open(site_url('changee/vendors_portal/quotation_form'), array('autocomplete' => 'off', 'class' => '_transaction_form'));
        }
        ?>
      <?php } ?>
      <div class="col-md-12">
        ...
        <div class="panel_s accounting-template estimate">
            ...
            <div class="tab-content">
              <div role="tabpanel" class="tab-pane ptop10 <?php if ($this->input->get('tab') != 'discussion') {
                ...
                <div class="row">
                  <div class="col-md-6">
                    <?php
                    $next_estimate_number = changee_max_number_estimates() + 1;
                        ...
                        <div class="input-group">
                          <span class="input-group-addon">
                            <?php if (isset($estimate)) { ?>
                              <a href="#" onclick="return false;" data-toggle="popover" data-container='._transaction_form' data-html="true" data-content="<label class='control-label'><?php echo _l('settings_sales_estimate_prefix'); ?></label><div class='input-group'><input name='s_prefix' type='text' class='form-control' value='<?php echo changee_pur_html_entity_decode($estimate->prefix); ?>'></div><button type='button' onclick='save_sales_number_settings(this); return false;' data-url='<?php echo admin_url('estimates/update_number_settings/' . $estimate->id); ?>' class='btn btn-info btn-block mtop15'><?php echo _l('submit'); ?></button>"><i class="fa fa-cog"></i></a>
                            <?php }
                            echo changee_pur_html_entity_decode($prefix);
                            ?>
                          </span>
                          <input type="text" name="number" class="form-control" value="<?php echo changee_pur_html_entity_decode($_estimate_number); ?>" data-isedit="<?php echo changee_pur_html_entity_decode($isedit); ?>" data-original-number="<?php echo changee_pur_html_entity_decode($data_original_number); ?>">
                          <?php if ($format == 3) { ?>
                            <span class="input-group-addon">
                              <span id="prefix_year" class="format-n-yy"><?php echo changee_pur_html_entity_decode($yy); ?></span>
                            </span>
                          <?php } else if ($format == 4) { ?>
                            <span class="input-group-addon">
                              <span id="prefix_month" class="format-mm-yyyy"><?php echo changee_pur_html_entity_decode($mm); ?></span>
                              /
                              <span id="prefix_year" class="format-mm-yyyy"><?php echo changee_pur_html_entity_decode($yyyy); ?></span>
                            </span>
                          <?php } ?>
                        </div>
                      </div>
                    ...
                    <div class="col-md-12">
                      ...
                      <?php echo changee_pur_render_date_input('date', 'estimate_add_edit_date', $value); ?>
                    </div>
                  ...
                  <div class="col-md-6">
                    <div class="row">
                      <div class="col-md-12 form-group">
                        ...
                        <select name="buyer" class="selectpicker" id="buyer" data-width="100%" data-live-search="true" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                          ...
                          <?php foreach ($staff as $st) { ?>
                            <option value="<?php echo changee_pur_html_entity_decode($st['staff_id']); ?>" <?php if (isset($estimate) && $estimate->buyer == $st['staff_id']) {
                                                                                                      echo 'selected';
                      ...
                      <div class="col-md-6">
                        ...
                        echo changee_pur_render_date_input('expirydate', 'estimate_add_edit_expirydate', $value); ?>
                      </div>
                ...
                <div role="tabpanel" class="tab-pane <?php if ($this->input->get('tab') === 'discussion') {
                  ...
                  foreach ($comments as $comment) {
                    ...
                    if ($comment['staffid'] != 0) {
                      ...
                      $comment_html .= changee_get_vendor_company_name(changee_get_vendor_user_id());
                    }
                    $comment_html .= '</b>';
          ...
          <div class="panel-body mtop10 invoice-item">
            <div class="row">
              ...
              if ($vendor_currency != 0 && !isset($estimate)) {
                $estimate_currency = changee_pur_get_currency_by_id($vendor_currency);
              }

              if (isset($estimate) && $estimate->currency != 0) {
                $estimate_currency = changee_pur_get_currency_by_id($estimate->currency);
              }
              ...
              <div class="col-md-8 <?php if ($estimate_currency->id == $base_currency->id) {
                ...
                <div class="col-md-2 pull-right">
                  <?php $currency_rate = 1;
                  if (isset($estimate) && $estimate->currency != 0) {
                    $currency_rate = changee_pur_get_currency_rate($base_currency->name, $estimate_currency->name);
...
<?php require 'modules/changee/assets/js/estimate_vendor_js.php'; ?>