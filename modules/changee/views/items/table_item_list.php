...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == db_prefix().'items.id') {
            $arr_images = $this->ci->changee_model->get_item_attachments($aRow[db_prefix().'items.id']);

            if(count($arr_images) > 0){
                if(file_exists(PURCHASE_MODULE_ITEM_UPLOAD_FOLDER .$arr_images[0]['rel_id'] .'/'.$arr_images[0]['file_name'])){
                    $_data = '<img class="images_w_table" src="' . site_url('modules/changee/uploads/item_img/' . $arr_images[0]['rel_id'] .'/'.$arr_images[0]['file_name']).'" alt="'.$arr_images[0]['file_name'] .'" >';
                ...
                if(is_numeric($aRow['from_vendor_item'])){
                    $vendor_image = $this->ci->changee_model->get_vendor_item_file($aRow['from_vendor_item']);
                    if(count($vendor_image) > 0){
                        if(file_exists(PURCHASE_PATH.'vendor_items/' .$aRow['from_vendor_item'] .'/'.$vendor_image[0]['file_name'])){
                            $_data = '<img class="images_w_table" src="' . site_url('modules/changee/uploads/vendor_items/' . $vendor_image[0]['rel_id'] .'/'.$vendor_image[0]['file_name']).'" alt="'.$vendor_image[0]['file_name'] .'" >';
                        }
                    }
                }
            }
         ...
         if($aColumns[$i] == 'commodity_code') {
             $code = '<a href="' . admin_url('changee/commodity_detail/' . $aRow[db_prefix().'items.id'] ).'" onclick="init_commodity_detail('.$aRow[db_prefix().'items.id'].'); return false;">' . $aRow['commodity_code'] . '</a>';
              $code .= '<div class="row-options">';

            $code .= '<a href="' . admin_url('changee/commodity_detail/' . $aRow[db_prefix().'items.id'] ).'" onclick="init_commodity_detail('.$aRow[db_prefix().'items.id'].'); return false;">' . _l('view') . '</a>';
            ...
            if (has_permission('changee_items', '', 'delete') || is_admin()) {
                $code .= ' | <a href="' . admin_url('changee/delete_commodity/' . $aRow[db_prefix().'items.id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            ...
            if($aRow['unit_id'] != null){
                $_data = changee_get_unit_type_item($aRow['unit_id']) != null ? changee_get_unit_type_item($aRow['unit_id'])->unit_name : '';
            }else{
                $data = '';
            }
            ...
            $vendor_item = $this->ci->changee_model->get_item_of_vendor($aRow['from_vendor_item']);
            if(isset($vendor_item->vendor_id)){ 
                $vendor_currency_id = changee_get_vendor_currency($vendor_item->vendor_id);
            ...
            $tax ='';
            $tax_rate = changee_get_tax_rate_item($aRow['tax']);
            $tax_rate2 = changee_get_tax_rate_item($aRow['tax2']);
            if($aRow['tax']){
            ...
            if($aRow['group_id'] != null){
                $_data = changee_get_group_name_item($aRow['group_id']) != null ? changee_get_group_name_item($aRow['group_id'])->name : '';
            }else{
                $_data = '';
            }
        }elseif($aColumns[$i] == '1'){
            ...
            $vendor_item = $this->ci->changee_model->get_item_of_vendor($aRow['from_vendor_item']);
            if(isset($vendor_item->vendor_id)){
                $_data = '<a href="'.admin_url('changee/vendor/'. $vendor_item->vendor_id).'">'.changee_get_vendor_company_name($vendor_item->vendor_id).'</a>';
            }else{
                $_data ='';
            }
