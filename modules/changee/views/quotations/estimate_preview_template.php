   ...
   <div class="panel_s">
      <div class="panel-body">
       ...
       <?php } ?>
            ...
            <div class="horizontal-tabs">
               <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                  ...
                  <li role="presentation">
                     <a href="#tab_tasks" onclick="init_rel_tasks_table(<?php echo changee_pur_html_entity_decode($estimate->id); ?>,'pur_quotation'); return false;" aria-controls="tab_tasks" role="tab" data-toggle="tab">
                     <?php echo _l('tasks'); ?>
                     </a>
                  </li> 
         ...
         <div class="row">
            <div class="col-md-6">
              <p class="bold mtop15" ><?php echo _l('vendor').': '?><a href="<?php echo admin_url('changee/vendor/'.$estimate->vendor->userid); ?>"><?php echo changee_pur_html_entity_decode($estimate->vendor->company); ?></a></p>
            ...
            <div class="col-md-6">
              ...
              <?php if($estimate->status != 2){ ?>
                  ...
                  <?php if(has_permission('changee_quotations','','edit')){ ?>
                  <a href="<?php echo admin_url('changee/estimate/'.$estimate->id); ?>" class="btn btn-default btn-with-tooltip" data-toggle="tooltip" title="<?php echo _l('edit_estimate_tooltip'); ?>" data-placement="bottom"><i class="fa fa-pencil-square"></i></a>
                  <?php } ?>
                <?php } ?>
                  ...
                  <ul class="dropdown-menu dropdown-menu-right">
                     <li class="hidden-xs"><a href="<?php echo admin_url('changee/purestimate_pdf/'.$estimate->id.'?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a></li>
                     <li class="hidden-xs"><a href="<?php echo admin_url('changee/purestimate_pdf/'.$estimate->id.'?output_type=I'); ?>" target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                     <li><a href="<?php echo admin_url('changee/purestimate_pdf/'.$estimate->id); ?>"><?php echo _l('download'); ?></a></li>
                     <li>
                        <a href="<?php echo admin_url('changee/purestimate_pdf/'.$estimate->id.'?print=true'); ?>" target="_blank">
                        <?php echo _l('print'); ?>
                        </a>
                     </li>
                  </ul>
              ...
              <a href="#" onclick="send_quotation('<?php echo changee_pur_html_entity_decode($estimate->id); ?>'); return false;" class="btn btn-success pull-right mleft5 mright5" ><i class="fa fa-envelope" data-toggle="tooltip" title="<?php echo _l('send_a_quote') ?>"></i></a>

              <?php if(has_permission('changee_estimate_change_approve_status', '', 'edit') && $check_approval_setting){ ?>
                 <select name="status" id="status" class="selectpicker pull-right mright10" onchange="change_status_pur_estimate(this,<?php echo changee_pur_html_entity_decode($estimate->id); ?>); return false;" data-live-search="true" data-width="35%" data-none-selected-text="<?php echo _l('pur_change_status_to'); ?>">
                   <option value=""></option>
                   <option value="1" class="<?php if($estimate->status == 1) { echo 'hide';}?>"><?php echo _l('changee_draft'); ?></option>
                   <option value="2" class="<?php if($estimate->status == 2) { echo 'hide';}?>"><?php echo _l('changee_approved'); ?></option>
                                    ...
                                    <li>
                                      ...
                                      <li>
                                        <div class="row text-right col-md-12">
                                         ...
                                         <a href="#" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="deny_request(<?php echo changee_pur_html_entity_decode($estimate->id); ?>); return false;" class="btn btn-warning"><?php echo _l('deny'); ?></a></div>
                                      </li>
                                   </ul>
                                </div>
                          <?php }
               ...
               <div class="col-md-12">
                  ...
                  if(isset($attachments) && count($attachments) > 0) { 
                     foreach($attachments as $value){
                       ...
                       $path = get_upload_path_by_type('changee').'pur_quotation/'.$value['rel_id'].'/'.$value['file_name'];
                     ...
                     <div class="col-md-6 col-sm-6">
                        <h4 class="bold">
                           <?php
                              ...
                              ?>
                           <a href="<?php echo admin_url('changee/estimate/'.$estimate->id); ?>">
                           <span id="estimate-number">
                           <?php echo changee_format_pur_estimate_number($estimate->id); ?>
                           </span>
                           </a>
                        </h4>
                        <address>
                           ...
                           <table class="table items items-preview estimate-items-preview" data-type="estimate">
                              ...
                              <tbody class="ui-sortable">
                                 ...
                                 <tr nobr="true" class="sortable">
                                    ...
                                    ?></strong></td>
                                    <td align="right"  width="12%"><?php echo changee_pur_html_entity_decode($es['quantity']); ?></td>
                                    ...
                                    <td class="amount" width="12%" align="right"><?php echo changee_pur_html_entity_decode($es['discount_%'].'%'); ?></td>
...
<div class="modal fade" id="send_quotation" tabindex="-1" role="dialog">
  <div class="modal-dialog">
      <?php echo form_open_multipart(admin_url('changee/send_quotation'),array('id'=>'send_quotation-form')); ?>
              ...
              <div class="row">
                ...
                <div class="col-md-12">
                  <div class="checkbox checkbox-primary">
                      ...
                      <label for="attach_pdf"><?php echo _l('attach_changee_quotation_pdf'); ?></label>
                  </div>
                </div>
...
<?php require 'modules/changee/assets/js/estimate_preview_template_js.php';?>
