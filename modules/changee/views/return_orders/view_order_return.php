...
<div class="col-md-12 no-padding">
	<div class="panel_s">
								...
								<div class="pull-right">
									...
									<div class="btn-group">
										...
										<ul class="dropdown-menu dropdown-menu-right">
											<li class="hidden-xs"><a href="<?php echo admin_url('changee/order_return_pdf/'.$order_return->id.'?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a></li>
											<li class="hidden-xs"><a href="<?php echo admin_url('changee/order_return_pdf/'.$order_return->id.'?output_type=I'); ?>" target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
											<li><a href="<?php echo admin_url('changee/order_return_pdf/'.$order_return->id); ?>"><?php echo _l('download'); ?></a></li>
											<li>
												<a href="<?php echo admin_url('changee/order_return_pdf/'.$order_return->id.'?print=true'); ?>" target="_blank">
													<?php echo _l('print'); ?>
												</a>
											</li>
										</ul>
									...
									<div class="btn-group">
					                     ...
					                     <ul class="dropdown-menu dropdown-menu-right">
					                     	<?php if(has_permission('changee_order_return','','edit') && ( $order_return->status == 'confirm' || $order_return->status == 'processing') && changee_get_order_return_remaining_refund($order_return->id) > 0){ ?>
						                     	<li>
					                        ...
					                        <?php foreach($statuses as $status){ ?>
					                        	<?php if($status != $order_return->status){ ?>
							                        <li>
							                           <a href="<?php echo admin_url('changee/mark_return_order_as/'.$status.'/'.$order_return->id); ?>"><?php echo _l('invoice_mark_as',_l('pur_'.$status)); ?></a>
							                        </li>
							                    <?php } ?>
						                    <?php } ?>    
					                        <?php } ?>
					                        ...
					                        <li>
					                           <a href="<?php echo admin_url('changee/delete_order_return/'.$order_return->id); ?>" class="text-danger delete-text _delete"><?php echo _l('delete_invoice'); ?></a>
					                        </li>
					                        <?php } ?>
						...
						<div class="row">
							<div class="col-md-6">
								<h4 class="bold">
									<span id="invoice-number">
										<?php echo changee_pur_html_entity_decode($order_return->order_return_number .' - '.$order_return->order_return_name); ?>
									</span>
								</h4>
								<address>
								...
								<?php }elseif($order_return->rel_type == 'purchasing_return_order'){ ?>
									<p class="no-mbot">
										<span class="bold">
											...
											<a href="<?php echo admin_url('changee/changee_order/'.$order_return->rel_id) ?>" ><?php echo $order_return->order_number; ?></a>
										</span>
										<h5 class="bold">
										</h5>
									</p>
							...
							<div class="col-sm-6 text-right">
								<?php if($order_return->rel_type == 'manual' || $order_return->rel_type == 'sales_return_order'){ ?>
									...
									<address>
										<?php echo changee_pur_html_entity_decode(get_company_name($order_return->company_id)) ?>
									</address>
								<?php }else{ ?>
									<span class="bold"><?php echo _l('pur_vendor'); ?>:</span>
									<address>
										<?php echo changee_pur_html_entity_decode(changee_get_vendor_company_name($order_return->company_id)) ?>
									</address>
								<?php } ?>
								<span class="bold"><?php echo _l('email'); ?>:</span>
									<?php echo changee_pur_html_entity_decode($order_return->email) ?><br>
								<span class="bold"><?php echo _l('phonenumber'); ?>:</span>
									<?php echo changee_pur_html_entity_decode($order_return->phonenumber) ?>
							...
							<div class="col-md-12">
									...
									<table class="table items items-preview estimate-items-preview" data-type="estimate">
										<thead>
											<tr>
												...
												<th align="right" colspan="1"><?php echo changee_pur_html_entity_decode($rate_label) ?></th>
										...
										<tbody class="ui-sortable">
											...
											foreach ($order_return_detail as $delivery => $order_return_value) {
												...
												$commodity_code = changee_pur_get_commodity_name($order_return_value['commodity_code']) != null ? changee_pur_get_commodity_name($order_return_value['commodity_code'])->commodity_code : '';
												$commodity_name = changee_pur_get_commodity_name($order_return_value['commodity_code']) != null ? changee_pur_get_commodity_name($order_return_value['commodity_code'])->description : '';

												$unit_name = '';
												if(is_numeric($order_return_value['unit_id'])){
													$unit_name = changee_get_unit_type_item($order_return_value['unit_id']) != null ? ' '.changee_get_unit_type_item($order_return_value['unit_id'])->unit_name : '';
												}

												$commodity_name = $order_return_value['commodity_name'];
												if(strlen($commodity_name) == 0){
													$commodity_name = changee_pur_get_item_variatiom($order_return_value['commodity_code']);
												}

												?>

												<tr>
													<td ><?php echo changee_pur_html_entity_decode($delivery) ?></td>
													<td ><?php echo changee_pur_html_entity_decode($commodity_name) ?></td>
													<td class="text-right"><?php echo changee_pur_html_entity_decode($quantity).$unit_name ?></td>
													<td class="text-right"><?php echo app_format_money((float)$unit_price,'') ?></td>

													<?php echo  changee_pur_render_taxes_html(changee_pur_convert_item_taxes($order_return_value['tax_id'], $order_return_value['tax_rate'], $order_return_value['tax_name']), 15); ?>
										...
										<table class="table text-right">
											<tbody>
												...
												<?php if(isset($order_return) && $tax_data['html_currency'] != ''){
													echo changee_pur_html_entity_decode($tax_data['html_currency']);
												} ?>
												...
												<?php $refunded_amount = changee_get_total_order_return_refunded($order_return->id); ?>
								...
								<?php if($order_return->return_reason != ''){ ?>
									<div class="col-md-12 row mtop15">
										...
										<p><?php echo changee_pur_html_entity_decode($order_return->return_reason); ?></p>
									</div>
								<?php } ?>
								...
								<?php if($order_return->admin_note != ''){ ?>
									<div class="col-md-12 row mtop15">
										...
										<p><?php echo changee_pur_html_entity_decode($order_return->admin_note); ?></p>
									</div>
								<?php } ?>
								<?php if($order_return->return_policies_information != ''){ ?>
									<div class="col-md-12 row mtop15">
										<p class="bold text-muted"><?php echo _l('pur_return_policies_information'); ?></p>
										<p><?php echo changee_pur_html_entity_decode($order_return->return_policies_information); ?></p>
									</div>
								<?php } ?>
			               ...
			               <table class="table table-bordered no-mtop">
			                  ...
			                  <tbody>
			                     ...
			                     <tr>
			                        ...
			                        <td>
			                           <?php if(has_permission('changee_order_return','','delete')) { ?>
			                           <a href="<?php echo admin_url('changee/delete_order_return_refund/'.$refund['id'].'/'.$refund['order_return_id']); ?>"
			                              class="pull-right text-danger _delete">
			                           <i class="fa fa-trash"></i>
			                           </a>
			                           <?php } ?>
			                           <?php if(has_permission('changee_order_return','','edit')) { ?>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-footer">
									...
									<button onclick="sign_request(<?php echo changee_pur_html_entity_decode($order_return->id); ?>);" autocomplete="off" class="btn btn-success sign_request_class"><?php echo _l('e_signature_sign'); ?></button>
								</div>
							</div>
						</div>
					</div>
		...
		<?php require 'modules/changee/assets/js/order_returns/view_order_return_js.php';?>
	</body>
	</html>