...
<div class="col-md-12 no-padding animated fadeIn">
    <div class="panel_s">
        <?php echo form_open(admin_url('changee/' . (isset($refund) ? 'edit_order_return_refund' : 'create_order_return_refund') .'/'. (isset($refund) ? $refund->id.'/'.$refund->order_return_id : $order_return->id)), ['id'=>'order_return_refund_form']); ?>
        <div class="panel-body">
            <h4 class="no-margin"><?php echo _l('refund'); ?> (<?php echo changee_pur_html_entity_decode($order_return->order_return_name); ?>)</h4>
            <hr class="hr-panel-heading" />
            <div class="row">
                <div class="col-md-6">

                    <?php 
                    $refunded_amount = changee_get_total_order_return_refunded($order_return->id);
                    $remaining_refund =  changee_get_order_return_remaining_refund($order_return->id);
