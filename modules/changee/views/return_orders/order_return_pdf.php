...
$invoice_info .= changee_get_vendor_company_name($order_return->company_id).'<br />';
...
foreach ($order_return->order_return_detail as $key => $order_return_detail) {
	...
	$itemHTML .= '<td class="description" align="left;" width="20%">';
			...
			if(is_numeric($order_return_detail['unit_id'])){
				$unit_name = changee_get_unit_type_item($order_return_detail['unit_id']) != null ? ' '.changee_get_unit_type_item($order_return_detail['unit_id'])->unit_name : '';
			}
			/**
			...
			$item_tax = changee_pur_convert_item_taxes($order_return_detail['tax_id'], $order_return_detail['tax_rate'], $order_return_detail['tax_name']);
