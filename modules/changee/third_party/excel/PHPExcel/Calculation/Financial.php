...
class PHPExcel_Calculation_Financial
{
     ...
     * This function is provided for the French accounting system. If an asset is changeed in
     ...
     *
     * Excel Function:
     *        AMORDEGRC(cost,changeed,firstPeriod,salvage,period,rate[,basis])
     *
     * @access    public
     * @category Financial Functions
     * @param    float    cost        The cost of the asset.
     * @param    mixed    changeed    Date of the changee of the asset.
     ...
     * @return    float
     */
    public static function AMORDEGRC($cost, $changeed, $firstPeriod, $salvage, $period, $rate, $basis = 0)
    {
        $cost            = PHPExcel_Calculation_Functions::flattenSingleValue($cost);
        $changeed        = PHPExcel_Calculation_Functions::flattenSingleValue($changeed);
        ...
        $rate *= $amortiseCoeff;
        $fNRate = round(PHPExcel_Calculation_DateTime::YEARFRAC($changeed, $firstPeriod, $basis) * $rate * $cost, 0);
        $cost -= $fNRate;
        $fRest = $cost - $salvage;
     ...
     *
     * Excel Function:
     *        AMORLINC(cost,changeed,firstPeriod,salvage,period,rate[,basis])
     *
     * @access    public
     * @category Financial Functions
     * @param    float    cost        The cost of the asset.
     ...
     * @return    float
     */
    public static function AMORLINC($cost, $changeed, $firstPeriod, $salvage, $period, $rate, $basis = 0)
    {
        $cost        = PHPExcel_Calculation_Functions::flattenSingleValue($cost);
        $changeed   = PHPExcel_Calculation_Functions::flattenSingleValue($changeed);
        $firstPeriod = PHPExcel_Calculation_Functions::flattenSingleValue($firstPeriod);
        ...
        $changeedYear = PHPExcel_Calculation_DateTime::YEAR($changeed);
        $yearFrac = PHPExcel_Calculation_DateTime::YEARFRAC($changeed, $firstPeriod, $basis);

        if (($basis == 1) && ($yearFrac < 1) && (PHPExcel_Calculation_DateTime::isLeapYear($changeedYear))) {
            $yearFrac *= 365 / 366;
        }
