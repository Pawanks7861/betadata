...
<?php if(isset($client)){ ?>
   ...
   $(function(){
      init_rel_tasks_table(<?php echo changee_pur_html_entity_decode($client->userid); ?>,'customer');
      initDataTable('.table-table_contracts', admin_url+'changee/table_vendor_contracts/'+<?php echo changee_pur_html_entity_decode($client->userid); ?>);

      initDataTable('.table-table_pur_order', admin_url+'changee/table_vendor_pur_order/'+<?php echo changee_pur_html_entity_decode($client->userid); ?>);

      initDataTable('.table-table_pur_invoices', admin_url+'changee/table_vendor_pur_invoices/'+<?php echo changee_pur_html_entity_decode($client->userid); ?>);
...
$(function() {
    ...
    <?php } ?>
    _table_api = initDataTable('.table-vendor_contacts', admin_url + 'changee/vendor_contacts/' + customer_id, contactsNotSortable, contactsNotSortable);
    if(_table_api) {
          <?php if(is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1'){ ?>
        _table_api.on('draw', function () {
    ...
    if (app.options.company_is_required == 1) {
        vRules = {
            ...
            vendor_code: {
               required: true,
               remote: {
                url: site_url + "admin/changee/vendor_code_exists",
                type: 'post',
                data: {
    ...
    <?php if(isset($client)  && $group == 'changee_statement'){ ?>
...
function render_vendor_statement(){
    ...
    var statementUrl = admin_url+'changee/statement';
    var statementUrlParams = new Array();

    statementUrlParams['vendor_id'] = '<?php echo changee_pur_html_entity_decode($client->userid); ?>';
    ...
    $.get(statementUrl,function(response){
        ...
        $('#statement_pdf').attr('href',buildUrl(admin_url+'changee/statement_pdf',statementUrlParams));
        $('#send_statement_form').attr('action',buildUrl(admin_url+'changee/send_statement',statementUrlParams));

        statementUrlParams['print'] = true;
        $('#statement_print').attr('href',buildUrl(admin_url+'changee/statement_pdf',statementUrlParams));
...
function validate_contact_form() {
    ...
    appValidateForm('#contact-form', {
        ...
        email: {
            ...
            remote: {
                url: admin_url + "changee/contact_email_exists",
                type: 'post',
                data: {
                    email: function() {
...
function vendor_contact(client_id, contact_id) {
    ...
    requestGet('changee/form_contact/' + client_id + '/' + contact_id).done(function(response) {
...
function view_ic_file(id, rel_id) {
    "use strict";
      $('#ic_file_data').empty();
      $("#ic_file_data").load(admin_url + 'changee/file_pur_vendor/' + id + '/' + rel_id, function(response, status, xhr) {
          if (status == "error") {
...
function delete_ic_attachment(id) {
    "use strict";
    if (confirm_delete()) {
        requestGet('changee/delete_ic_attachment/' + id).done(function(success) {
            if (success == 1) {
