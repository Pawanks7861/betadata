<script>
  ...
  var estimate_id = '<?php echo changee_pur_html_entity_decode($estimate->id); ?>';
(function($) {
"use strict"; 
   var data_send_mail = {};
...
function change_status_pur_estimate(invoker,id){
  "use strict";
   $.post(admin_url+'changee/change_status_pur_estimate/'+invoker.value+'/'+id).done(function(reponse){
    reponse = JSON.parse(reponse);
    window.location.href = admin_url + 'changee/quotations/'+id;
...
function view_estimate_file(id, rel_id) {
  "use strict"; 
      ...
      $("#estimate_file_data").load(admin_url + 'changee/file_pur_estimate/' + id + '/' + rel_id, function(response, status, xhr) {
          if (status == "error") {
...
function delete_estimate_attachment(id) {
  "use strict"; 
    if (confirm_delete()) {
        requestGet('changee/delete_estimate_attachment/' + id).done(function(success) {
            if (success == 1) {
...
function add_contract_comment() {
  "use strict";
   ...
   function get_contract_comments() {
    ...
    requestGet('changee/get_comments/' + estimate_id+'/pur_quotation').done(function (response) {
