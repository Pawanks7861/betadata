...
fnServerParams, 
statistics_number_of_changee_orders, 
statistics_cost_of_changee_orders;
 var report_from = $('input[name="report-from"]');
 var report_to = $('input[name="report-to"]');
  var date_range = $('#date-range');
(function($) {
  "use strict";
  report_pur_inv = $('#list_changee_inv_report');
  report_po = $('#list_po_report');
  report_po_voucher = $('#list_po_voucher');
  report_import_goods = $('#list_import_goods');
  statistics_number_of_changee_orders = $('#number-changee-orders-report');
  statistics_cost_of_changee_orders = $('#cost-changee-orders-report');
  ...
  $('select[name="months-report"]').on('change', function() {
   ...
   $('.table-changee-inv-report').on('draw.dt', function() {
     var poReportsTable = $(this).DataTable();
     var sums = poReportsTable.ajax.json().sums;
     $(this).find('tfoot').addClass('bold');
     $(this).find('tfoot td').eq(0).html("<?php echo _l('invoice_total'); ?> (<?php echo _l('per_page'); ?>)");
 ...
 function init_report(e, type) {
  ...
  statistics_cost_of_changee_orders.addClass('hide');
  statistics_number_of_changee_orders.addClass('hide');

  $('select[name="months-report"]').selectpicker('val', 'this_month');
    // Clear custom date picker
      $('#currency').removeClass('hide');

      if (type != 'statistics_number_of_changee_orders' && type != 'statistics_cost_of_changee_orders') {
        report_from_choose.removeClass('hide');
      }
      if (type == 'list_import_goods') {
        report_import_goods.removeClass('hide');
      }else if(type == 'statistics_number_of_changee_orders'){
        $('#currency').addClass('hide');
        statistics_number_of_changee_orders.removeClass('hide');
        $('#year_requisition').removeClass('hide');
      }else if(type == 'statistics_cost_of_changee_orders'){
        statistics_cost_of_changee_orders.removeClass('hide');
        $('#year_requisition').removeClass('hide');
      ...
      }else if(type == 'changee_invoice_rp'){
...
function import_goods_report() {
 ...
 initDataTable('.table-import-goods-report', admin_url + 'changee/import_goods_report', false, false, fnServerParams);
}
...
function po_voucher_report() {
 ...
 initDataTable('.table-po-voucher-report', admin_url + 'changee/po_voucher_report', false, false, fnServerParams);
}
...
function po_report() {
 ...
 initDataTable('.table-po-report', admin_url + 'changee/po_report', false, false, fnServerParams);
}

function changee_inv_report() {
  "use strict";

 if ($.fn.DataTable.isDataTable('.table-changee-inv-report')) {
   $('.table-changee-inv-report').DataTable().destroy();
 }
 initDataTable('.table-changee-inv-report', admin_url + 'changee/changee_inv_report', false, false, fnServerParams);
}


function number_of_changee_orders_analysis() {
  "use strict";

  var data = {};
   data.year = $('select[name="year_requisition"]').val();
  $.post(admin_url + 'changee/number_of_changee_orders_analysis/', data).done(function(response) {
     response = JSON.parse(response);
      ...
      colors: [ '#119EFA','#ef370dc7','#15f34f','#791db2d1', '#DDDF00', '#24CBE5', '#64E572', '#FF9655', '#FFF263','#6AF9C4','#50B432','#0d91efc7','#ED561B']
        ...
        Highcharts.chart('container_number_changee_orders', {
         chart: {
             type: 'column'
         },
         title: {
             text: '<?php echo _l('number_of_changee_orders') ?>'
         },
         subtitle: {
             text: ''
         },
         ...
         series: [{
            type: 'column',
            colorByPoint: true,
            name: '<?php echo _l('changee_quantity') ?>',
            data: response,
            showInLegend: false
         }]
     });
...
function cost_of_changee_orders_analysis() {
  "use strict";

  var data = {};
   data.year = $('select[name="year_requisition"]').val();
   data.report_currency = $('select[name="currency"]').val();
  $.post(admin_url + 'changee/cost_of_changee_orders_analysis', data).done(function(response) {
     response = JSON.parse(response);
        Highcharts.setOptions({
      chart: {
          style: {
      ...
      colors: [ '#119EFA','#ef370dc7','#15f34f','#791db2d1', '#DDDF00', '#24CBE5', '#64E572', '#FF9655', '#FFF263','#6AF9C4','#50B432','#0d91efc7','#ED561B']
        ...
        Highcharts.chart('container_cost_changee_orders', {
         chart: {
             type: 'column'
         },
         title: {
             text: '<?php echo _l('cost_of_changee_orders') ?>'
         },
         subtitle: {
             text: ''
         },
...
function gen_reports() {
 ...
 if (!report_import_goods.hasClass('hide')) {
   import_goods_report();
 }else if (!statistics_number_of_changee_orders.hasClass('hide')) {
    number_of_changee_orders_analysis();
 }else if (!statistics_cost_of_changee_orders.hasClass('hide')) {
    cost_of_changee_orders_analysis();
