...
function confirm_order(el){
  ...
  if(r == true && order_id != ''){ 
    $.post(site_url + 'changee/vendors_portal/confirm_order/'+ order_id).done(function (response) { 
...
function update_delivery_status(el){
  var status = $(el).val();
  var order_id = '<?php echo changee_pur_html_entity_decode($pur_order->id); ?>';
  $.post(site_url + 'changee/vendors_portal/update_delivery_status/'+ order_id+'/'+status).done(function (response) { 
    response = JSON.parse(response);
    if(response.success == true || response.success == 'true'){
      if(response.success == true || response.success == 'true'){
...
function update_delivery_date(el){
  ...
  data.date = $(el).val();
  var order_id = '<?php echo changee_pur_html_entity_decode($pur_order->id); ?>';
  $.post(site_url + 'changee/vendors_portal/update_delivery_date/'+ order_id, data).done(function (response) {
    response = JSON.parse(response);
    if(response.success == true || response.success == 'true'){
