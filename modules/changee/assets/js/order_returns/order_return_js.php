<script>
	...
	(function($) {
	 ...
	 init_ajax_search('items','#item_select.ajax-search',undefined,admin_url+'changee/pur_commodity_code_search/rate');
	 pur_calculate_total();
...
$("body").on('change', 'select[name="return_type"]', function () {
	var return_type = $(this).val();
		...
		$.post(admin_url + 'changee/order_return_get_item_data/'+rel_id+'/'+rel_type+'/'+return_type).done(function(response){
...
function pur_add_item_to_preview(id) {
	...
	requestGetJSON('changee/get_item_by_id/' + id +'/'+true).done(function (response) {
...
function pur_get_item_row_template(name, commodity_name, quantity, unit_name, unit_price, taxname, commodity_code, unit_id, tax_rate, discount, item_key)  {
	...
	var d = $.post(admin_url + 'changee/get_order_return_row_template', {
		name: name,
...
function get_available_quantity(commodity_code_name, from_stock_name, available_quantity_name){
	...
	if(data.commodity_id != '' && data.warehouse_id != ''){
		$.post(admin_url + 'changee/get_quantity_inventory', data).done(function(response){
			response = JSON.parse(response);
...
function invoice_change(){
	...
	$.post(admin_url + 'changee/copy_invoices/'+invoice_id).done(function(response){
		response = JSON.parse(response);
...
$('select[name="delivery_note_id"]').on('change', function() {
	...
	$.post(admin_url + 'changee/packing_list_copy_delivery_note/'+delivery_note_id).done(function(response){
		response = JSON.parse(response);
...
$("body").on('change', '#company_id', function () {
	...
	if(company_id.length > 0){
		requestGetJSON('changee/pur_client_data/' + company_id +'/'+ rel_type).done(function (response) {
			$('input[name="email"]').val(response.email);
			$('input[name="phonenumber"]').val(response.phonenumber);
...
$('select[name="rel_id"]').on('change', function() {
	...
	var rel_id = $('select[name="rel_id"]').val();
	var rel_type = $('input[name="rel_type"]').val();
	$.post(admin_url + 'changee/order_return_get_item_data/'+rel_id+'/'+rel_type).done(function(response){
		response = JSON.parse(response);
		$('select[name="company_id"]').val(response.company_id).change();
		$('input[name="email"]').val(response.email);
		$('input[name="phonenumber"]').val(response.phonenumber);
