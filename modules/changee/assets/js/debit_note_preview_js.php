<script>
   ...
   function refund_debit_note() {
      ...
      $('#debit_note').load(admin_url + 'changee/refund_debit_note/' + debit_note_id);
   }

   function edit_refund(refund_id) {
      var debit_note_id = <?php echo $debit_note->id; ?>;
      $('#debit_note').load(admin_url + 'changee/refund_debit_note/' + debit_note_id + '/' + refund_id);
   }

   function delete_debit_note_attachment(id) {
    if (confirm_delete()) {
        requestGet('changee/delete_debit_attachment/' + id).done(function (success) {
            if (success == 1) {
