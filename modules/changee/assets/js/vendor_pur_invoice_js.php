...
(function($) {
  ...
  $("body").on('change', 'select[name="currency"]', function () {
    ...
    if(currency_id != ''){
      $.post(site_url + 'changee/vendors_portal/get_currency_rate/'+currency_id).done(function(response){
...
function contract_change(el){
  "use strict";
  if(el.value != ''){
    $.post(site_url + 'changee/vendors_portal/contract_change/'+el.value).done(function(response) {
      response = JSON.parse(response);
      $('select[name="pur_order"]').val(response.changee_order).change();
    });
  }
}
...
function pur_order_change(el){
  "use strict";
  if(el.value != ''){
    $.post(site_url + 'changee/vendors_portal/pur_order_change/'+el.value).done(function(response) {
      response = JSON.parse(response);
        if(response){ 
...
function init_pi_currency(id, callback) {
    ...
    if ($accountingTemplate.length || id) {
        ...
        requestGetJSON('changee/vendors_portal/get_currency/' + selectedCurrencyId)
...
function pur_add_item_to_preview(id) {
  ...
  requestGetJSON('changee/vendors_portal/get_item_by_id/' + id +'/'+ currency_rate).done(function (response) {
...
function pur_get_item_row_template(name, item_name, description, quantity, unit_name, unit_price, taxname,  item_code, unit_id, tax_rate, discount, item_key, currency_rate, to_currency)  {
  ...
  var d = $.post(site_url + 'changee/vendors_portal/get_changee_invoice_row_template', {
    name: name,
    item_name : item_name,
...
function init_ajax_search(type, selector, server_data, url) {
    ...
    if (ajaxSelector.length) {
        var options = {
            ajax: {
                url: (typeof (url) == 'undefined' ? site_url + 'changee/vendors_portal/get_relation_data' : url),
                data: function () {
                    var data = {};
                    data.type = type;
                    data.rel_id = '';
