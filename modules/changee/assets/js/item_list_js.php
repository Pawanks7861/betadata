...
var hidden_columns = [3,7];
  ...
  "use strict";
     ...
     $('select[name="group_id"]').on('change',function(){
      ...
      var data_select = {};
        data_select.group_id = $('select[name="group_id"]').val();
          ...
          // $.post(admin_url + 'changee/get_subgroup_fill_data',data_select).done(function(response){
          //    response = JSON.parse(response);
    ...
    _table_api = initDataTable(table_item_list, admin_url+'changee/table_item_list', [0], [0], '',  [1, 'desc']);


  init_commodity_detail();
  function init_commodity_detail(id) {
    load_small_table_item_proposal(id, '#proposal_sm_view', 'item_id', 'changee/get_item_data_ajax', '.proposal_sm');
  }
...
$('#hot-display-license-info').empty();
        ...
        if (response.commodityid) {
         if(typeof(expenseDropzone) !== 'undefined'){
          if (expenseDropzone.getQueuedFiles().length > 0) {
            expenseDropzone.options.url = admin_url + 'changee/add_commodity_attachment/' + response.commodityid;
            expenseDropzone.processQueue();
          } else {
            window.location.assign(response.url);
          }
  ...
  function delete_contract_attachment(wrapper, id) {
    ...
    if (confirm_delete()) {
       $.get(admin_url + 'changee/delete_commodity_file/' + id, function (response) {
...
function edit_commodity_item(invoker){
  ...
  $('#commodity_list-add-edit').modal('show');
      ...
      $('#commodity_list-add-edit select[name="changee_id"]').val($(invoker).data('changee_id')).change();
      ...
      /*get file url*/
              /*get description*/
        $.post(admin_url + 'changee/get_commodity_file_url/'+$(invoker).data('commodity_id')).done(function(response) {
            response = JSON.parse(response);
  ...
  function new_commodity_item(){
    ...
    $.post(admin_url + 'changee/get_commodity_barcode').done(function(response) {
      response = JSON.parse(response);
    ...
    $('#commodity_list-add-edit').modal('show');
      ...
      $('#commodity_list-add-edit select[name="group_id"]').val('').change();
      $('#commodity_list-add-edit select[name="changee_id"]').val('').change();
      $('#commodity_list-add-edit select[name="tax"]').val('').change();
...
function changee_delete_bulk_action(event) {
    "use strict";
      ...
      if (confirm_delete()) {
          ...
          if(mass_delete == true || ( change_item_selling_price == true && selling_price != '') || ( change_item_purchase_price == true && purchase_price != '') || clone_items == true){
              ...
              setTimeout(function() {
                  $.post(admin_url + 'changee/changee_delete_bulk_action', data).done(function() {
  ...
  "use strict";
    ...
    var item_selling_changee_checking = $('input[id="change_item_purchase_price"]').is(":checked");

    if(item_selling_changee_checking){
      $('input[id="mass_delete"]').prop("checked", false);
