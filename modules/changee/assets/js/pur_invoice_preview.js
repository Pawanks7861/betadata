...
function view_purinv_file(id, rel_id) {
  "use strict"; 
      ...
      $("#purinv_file_data").load(admin_url + 'changee/file_purinv/' + id + '/' + rel_id, function(response, status, xhr) {
          if (status == "error") {
...
function delete_purinv_attachment(id) {
  "use strict"; 
    if (confirm_delete()) {
        requestGet('changee/delete_purinv_attachment/' + id).done(function(success) {
            if (success == 1) {
...
function add_contract_comment() {
 ...
 function get_contract_comments() {
  ...
  requestGet('changee/get_comments/' + invoice_id+'/pur_invoice').done(function (response) {
