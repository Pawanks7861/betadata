<script>
var pur_order_id = '<?php echo changee_pur_html_entity_decode($estimate->id); ?>';
(function($) {
  "use strict"; 
   var data_send_mail = {};
  <?php if(isset($send_mail_approve)){ 
    ...
    data_send_mail.rel_id = <?php echo changee_pur_html_entity_decode($estimate->id); ?>;
    data_send_mail.rel_type = 'pur_order';
    data_send_mail.addedfrom = <?php echo changee_pur_html_entity_decode($estimate->addedfrom); ?>;
...
function pur_inv_payment_change(el){
  ...
  var invoice = $(el).val();
  if(invoice != '' ){
    $.post(admin_url+'changee/pur_inv_payment_change/'+invoice).done(function(reponse){
    ...
    alert_float('warning', '<?php echo _l('please_select_changee_invoice'); ?>');
  }
}
...
function change_status_pur_order(invoker,id){
  "use strict"; 
   $.post(admin_url+'changee/change_status_pur_order/'+invoker.value+'/'+id).done(function(reponse){
    reponse = JSON.parse(reponse);
    window.location.href = admin_url + 'changee/changee_order/'+id;
    alert_float('success',reponse.result);
  });
}

//preview changee order attachment
...
function view_purorder_file(id, rel_id) {
  "use strict"; 
      ...
      $("#purorder_file_data").load(admin_url + 'changee/file_purorder/' + id + '/' + rel_id, function(response, status, xhr) {
          if (status == "error") {
...
function delete_purorder_attachment(id) {
  "use strict"; 
    if (confirm_delete()) {
        requestGet('changee/delete_purorder_attachment/' + id).done(function(success) {
            if (success == 1) {
...
function copy_public_link(pur_order){
  ...
  if(link != ''){
    ...
    $.post(admin_url+'changee/copy_public_link/'+pur_order).done(function(reponse){
      reponse = JSON.parse(reponse);
      if(reponse.copylink != ''){
...
function send_request_approve(id){
  "use strict";
    var data = {};
    data.rel_id = <?php echo changee_pur_html_entity_decode($estimate->id); ?>;
    data.rel_type = 'pur_order';
    data.addedfrom = <?php echo changee_pur_html_entity_decode($estimate->addedfrom); ?>;
...
function convert_to_changee_inv(pur_order){
  "use strict";
  $.post(admin_url + 'changee/convert_po_payment/' + pur_order).done(function(response){
      response = JSON.parse(response);
...
function add_contract_comment() {
  "use strict";
   ...
   function get_contract_comments() {
    ...
    requestGet('changee/get_comments/' + pur_order_id+'/pur_order').done(function (response) {
...
function refresh_order_value(po_id){
  ...
  if(r == true && po_id != ''){
    $.post(admin_url + 'changee/refresh_order_value/'+ po_id).done(function (response) { 
      response = JSON.parse(response);
      window.location.reload();
      if(response.success == true){
