...
function contract_change(el){
  "use strict";
  if(el.value != ''){
    $.post(admin_url + 'changee/contract_change/'+el.value).done(function(response) {
      response = JSON.parse(response);
...
function pur_order_change(el){
  "use strict";
  if(el.value != ''){
    $.post(admin_url + 'changee/pur_order_change/'+el.value).done(function(response) {
      response = JSON.parse(response);
        if(response){ 
...
function pur_vendor_change(el){
  "use strict";
  if(el.value != ''){
    $.post(admin_url + 'changee/pur_vendors_change/'+el.value).done(function(response) {
      response = JSON.parse(response);
...
function tax_rate_change(el){
  ...
  var total = $('#total').val();
  if(el.value != ''){
    $.post(admin_url + 'changee/tax_rate_change/'+el.value).done(function(response) {
      response = JSON.parse(response);
...
function pur_get_item_row_template(name, item_name, description, quantity, unit_name, unit_price, taxname,  item_code, unit_id, tax_rate, discount, item_key, currency_rate, to_currency)  {
  ...
  var d = $.post(admin_url + 'changee/get_changee_invoice_row_template', {
    name: name,
    item_name : item_name,
